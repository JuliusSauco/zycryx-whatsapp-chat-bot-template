...
4. **QR de vinculacion (corregido 2026-08-18).** Baileys 7 deprecó `printQRInTerminal` (no-op). El campo `qr` de `connection.update` ahora se transforma en imagen y se publica de forma autenticada en la consola web, que también permite elegir vinculación por código y reemplazar la sesión de manera controlada.
