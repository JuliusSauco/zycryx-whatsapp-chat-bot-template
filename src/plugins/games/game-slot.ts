...
export default defineSdkPlugin({
    ...
    if (remaining > 0) return sdk.reply.message('games.slot.cooldown', {
    ...
    if (saldo < cantidad) return sdk.reply.message('games.slot.notEnough', {
    ...
    const msg = await conn.sendMessage(m.chat, {text: render(renderRandom(emojis), sdk.content)}, {quoted: m});
    if (!msg) return;

    for (let i = 0; i < 6; i++) {
        await delay(300);
        if (i < 5) {
            await conn.sendMessage(m.chat, {text: render(renderRandom(emojis), sdk.content), edit: msg.key});
        } else {
            final = [
            ...
            await conn.sendMessage(m.chat, {text: render(final, sdk.content), edit: msg.key});
        }
    }
    if (!final) return;
    ...
    if (resultado === 'triple') {
        ganancia = cantidad * 3;
        textoFinal = sdk.content.renderMessage('games.slot.triple', {
            amount: formatThousandsDot(ganancia),
            resource: tipoBonito(tipo)
        });
    } else if (resultado === 'doble') {
        ganancia = cantidad;
        textoFinal = sdk.content.renderMessage('games.slot.double', {
            amount: formatThousandsDot(ganancia),
            resource: tipoBonito(tipo)
        });
    } else {
        ganancia = -cantidad;
        textoFinal = sdk.content.renderMessage('games.slot.lose', {
...
function render(matriz: SlotMatrix, pluginContent: PluginContentSdk) {
    return pluginContent.renderMessage('games.slot.board', {
