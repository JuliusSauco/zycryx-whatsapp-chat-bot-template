...
const GIF_FOLDER = path.join(process.cwd(), 'resources', 'media', 'reaction-gifs', 'les')

export default definePlugin({
    help: ['msg-gif-les'],
    tags: ['fun'],
    command: /^(lesbian|sexo-lesbiano|lesbianas|sexo-mujeres)$/i,
    register: false,
    ...
    try {
        ...
        if (!mp4s.length) {
            await m.reply('⚠️ Para enviarlo como “mensaje” (inline), convierte los GIFs a MP4 y guárdalos en `resources/media/reaction-gifs/les`.\n\nEjemplo ffmpeg:\nffmpeg -i input.gif -vf "fps=15,scale=320:-2:flags=lanczos" -an -c:v libx264 -pix_fmt yuv420p -movflags +faststart -crf 30 -preset veryfast output.mp4')
            return
        }

        const randomFile = pickRandomFile(mp4s)
        const filePath = path.join(GIF_FOLDER, randomFile)
        const texto = `👩‍❤️‍💋‍👩 *${senderTag}* tiene intenciones lesbicas 🫶 y ganas de hacer tijerasos ✂️ con *${mentionedTags.join(', ')}* 👭`
