...
const NSFW_GIF_FOLDER = path.join(process.cwd(), 'resources', 'media', 'reaction-gifs', 'dd', 'nsfw');
const NSFW_GIF_FOLDER_LABEL = 'resources/media/reaction-gifs/dd/nsfw';
...
export default defineSdkPlugin({
    ...
    async execute(m, {sdk}) {
        try {
            ...
            if (!rawTarget) {
                await sdk.reply.message('messages.gifDedeo.missingTarget');
                return;
            }
            ...
            const caption = sdk.content.renderMessage('messages.gifDedeo.caption', {
                sender: sender.tag,
                target: target.tag,
            });
