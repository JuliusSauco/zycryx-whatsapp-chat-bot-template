...
const randomBytes: string = crypto.randomBytes(5).toString("hex");
const urlRegex = /^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/;

async function webp2mp4(source: Buffer | string): Promise<string | string[]> {
    ...
    try {
        return await convert({
            ...
            ...(isUrl ? {
                ...
                file: new Blob([source as Buffer]),
                filename: randomBytes + "." + (await fileTypeFromBuffer(source as Buffer))!.ext
            })
        });
    } catch (error) {
...
async function webp2png(source: Buffer | string): Promise<string | string[]> {
    const isUrl = typeof source === 'string' && urlRegex.test(source);
