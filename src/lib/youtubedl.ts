...
const ogmp3 = {
    ...
    utils: {
        ...
        enc_url: (url: string, separator: string = ","): string => {
            const codes = [];
    ...
    async checkStatus(id: string): Promise<OgmpRequestResponse> {
        try {
    ...
    download: async (link: string, format?: string, type: string = 'video'): Promise<OgmpResponse> => {
        if (!link) {
            return {
                status: false,
                code: 400,
...
export {ogmp3};
export type {OgmpResponse, OgmpResult};
